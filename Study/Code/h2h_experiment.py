...
def filter_features(features, condition):
    if condition == 'A_All_H2H':
        return features
    elif condition == 'B_Only_Goals':
        return [f for f in features if not f.startswith('h2h_') or f in ['h2h_avg_goals_home', 'h2h_avg_goals_away']]
    elif condition == 'C_No_H2H':
...
def main():
    ...
    conditions = ['A_All_H2H', 'B_Only_Goals', 'C_No_H2H']
    results = {}
